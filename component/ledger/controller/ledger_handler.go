...
func (h *LedgerHandler) staffOrAdminOnly(w http.ResponseWriter, r *http.Request) bool {
	...
	if claims.Role != "staff" && claims.Role != "admin" {
		web.RespondErrorMessage(w, http.StatusForbidden, "only staff or admin can access this resource")
		return false
	}
	return true
}
