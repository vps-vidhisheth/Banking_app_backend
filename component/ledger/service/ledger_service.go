...
// // CreateLedger supports running inside or outside a transaction
// func (s *LedgerService) CreateLedger(ctx context.Context, accountID uuid.UUID, amount float64, ledgerType, description string, bankFromID, bankToID *uuid.UUID, tx *gorm.DB) (*model.Ledger, error) {
// 	if amount <= 0 {
...
// 	// Run inside provided transaction if available
// 	if tx != nil {
// 		if err := s.repo.WithTransaction(tx).Create(ctx, entry); err != nil {
// 			return nil, err // rollback handled by caller
// 		}
// 		return entry, nil
// 	}

// 	// Otherwise create a transaction for this ledger entry
// 	err := s.db.WithContext(ctx).Transaction(func(t *gorm.DB) error {
// 		if err := s.repo.WithTransaction(t).Create(ctx, entry); err != nil {
// 			return err // rollback
// 		}
// 		return nil // commit
// 	})
...
// // GetAllLedgers supports pagination using utils
...
// // DeleteLedger runs inside a transaction
// func (s *LedgerService) DeleteLedger(ctx context.Context, id uuid.UUID) error {
// 	return s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
// 		if err := s.repo.WithTransaction(tx).DeleteByID(ctx, id); err != nil {
// 			return err // rollback
// 		}
// 		return nil // commit
// 	})
...
// // GetLedger by ID (read-only, no transaction needed)
// func (s *LedgerService) GetLedger(ctx context.Context, id uuid.UUID) (*model.Ledger, error) {
// 	return s.repo.GetByID(ctx, id)
// }

// // GetNetBankTransfer calculates net transfer between two banks (read-only)
...
// 		Model(&model.Ledger{}).
// 		Select("COALESCE(SUM(amount), 0)").
// 		Where("bank_from_id = ? AND bank_to_id = ?", bankToID, bankFromID).
