...
func (h *TransactionHandler) GetTransactionsHandler(w http.ResponseWriter, r *http.Request) {
	...
	transactions, total, err := h.service.GetTransactionsByAccountPaginated(accountID, pagination.Limit, pagination.Offset)
	if err != nil {
		web.RespondErrorMessage(w, http.StatusInternalServerError, err.Error())
		return
	}

	web.RespondJSON(w, http.StatusOK, utils.PaginatedResponse(transactions, total, pagination.Limit, pagination.Offset))
}

// GetNetTransfersHandler
func (h *TransactionHandler) GetNetTransfersHandler(w http.ResponseWriter, r *http.Request) {
	accountIDStr := mux.Vars(r)["id"]
	accountID, err := uuid.Parse(accountIDStr)
	if err != nil {
...
// GetAllTransactions (all accounts with optional account_id filter)
func (h *TransactionHandler) GetAllTransactions(w http.ResponseWriter, r *http.Request) {
	pagination := utils.GetPaginationParams(r, 10, 0)
	accountParam := r.URL.Query().Get("account_id")
	...
	if accountParam != "" {
		...
		transactions, total, err = h.service.GetAllTransactionsPaginated(pagination.Limit, pagination.Offset)
	}
